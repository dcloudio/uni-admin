...
<script setup>
  ...
  import { mutations as uniIdMutations, store as uniIdStore } from '@/uni_modules/uni-id-pages/common/store.js';
  ...
  const { langs, logo, themes } = config.navBar;
  const popupMenuOpened = ref(false);
  const themeMenuOpened = ref(false);
  const langMenuOpened = ref(false);
  const mpCapsule = ref(0);
  const langIndex = ref(0);
  const errorLogsPopup = ref(null);
  ...
  const showErrorLogs = () => {
    popupMenuOpened.value = false;
    errorLogsPopup.value.open();
  };
  ...
  const toggleSidebar = () => {
    if (!props.showLeftWindow) uni.showLeftWindow();
    else uni.hideLeftWindow();
  };
  const togglePopupMenu = () => {
    popupMenuOpened.value = !popupMenuOpened.value;
    if (popupMenuOpened.value) {
      themeMenuOpened.value = false;
      langMenuOpened.value = false;
    }
  };
  const toggleThemeMenu = () => {
    themeMenuOpened.value = !themeMenuOpened.value;
    if (themeMenuOpened.value) {
      popupMenuOpened.value = false;
      langMenuOpened.value = false;
    }
  };
  const toggleLangMenu = () => {
    langMenuOpened.value = !langMenuOpened.value;
    if (langMenuOpened.value) {
      popupMenuOpened.value = false;
      themeMenuOpened.value = false;
    }
  };
  ...
  const changeLanguage = (selectedIndex) => {
    const index = !selectedIndex || selectedIndex < 0 ? 0 : selectedIndex;
    const lang = langs[index].lang || 'zh-Hans';
    if (!proxy.$i18n) {
      uni.showToast({ icon: 'error', title: '暂不支持', duration: 2000 });
      return;
    }
  ...
  const linkTo = () => uni.reLaunch({ url: '/' });
  const changeTheme = (index) => {
    const value = themes[index].value || 'default';
    if (theme.value !== value) store.commit('app/SET_THEME', value);
    themeMenuOpened.value = false;
  };

  onMounted(() => {
    // #ifdef MP
    mpCapsule.value = uni.getMenuButtonBoundingClientRect().width;
    // #endif

    // #ifdef H5
    const locale = uni.getLocale();
    nextTick(() => changeLanguage(langs.findIndex((item) => item.lang === locale)));
    // #endif
  });
</script>
