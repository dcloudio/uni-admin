  ...
  async run(type, date, reset, offset = -1) {
    ...
    if (statPayOrdersList.length > 0) {
      ...
      for (let i = 0; i < statPayOrdersList.length; i++) {
        ...
        let { appid, version, platform, channel } = item._id; // 从 _id 属性中解构出 appid、version、platform 和 channel 属性
        if (!appid) {
          continue;
        }
        let { status_str } = item; // 解构出 status_str 属性
        let key = `${appid}-${version}-${platform}-${channel}`; // 拼接 key 字符串
        if (!statDta[key]) {
          // 如果 statDta 对应的 key 不存在

          statDta[key] = {
            // 创建一个新的对象，赋值给 statDta[key]
            appid,
            version,
            platform,
            channel,
    ...
    for (let key in statDta) {
      ...
      let { appid, version, platform, channel, status: statusData } = item;
...
function getNowDate(date = new Date(), targetTimezone = 8, dimension) {
  ...
  const { year, month, day, hour, minute, second } = getFullTime(date);
  // 现在的时间
  let date_str;
  if (dimension === 'month') {
