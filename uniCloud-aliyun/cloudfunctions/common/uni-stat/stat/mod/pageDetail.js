...
module.exports = class PageDetail extends BaseMod {
   ...
   */
  async getPageDetail({ appid, pageId, url } = {}) {
    ...
    if (!pageDetailData) {
      const pageDetailInfo = await this.getCollection(this.tableName)
        .where({
          appid,
          page_id: pageId,
          page_link: url,
   ...
   * @return {Object} pageDetailInfo 页面详情信息
   */
  async getPageDetailAndCreate({ appid, pageId, url, title } = {}) {
   ...
   * @return {Object} pageDetailInfo 页面详情信息
   */
  async getPageDetailByPageRules({ appid, pageUrl, pageTitle, pageId, pageRules } = {}) {
  ...
  getPageDetailUrlByRules(url, pageRules) {
    ...
    const matchQuery = Object.keys(matchRulePrams)
      .map((key) => key + '=' + matchRulePrams[key])
      .join('&');
