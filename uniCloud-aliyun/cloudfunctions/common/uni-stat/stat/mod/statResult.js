...
module.exports = class StatResult extends BaseMod {
  ...
  async getWeekOrMonthData(data) {
    ...
    // 查询活跃设备数
    const statVisitDeviceRes = await this.getCollection(this.activeDevices.tableName)
      .where({
        ...matchCondition,
        dimension: this.fillType,
      })
    ...
    const totalDeviceRes = await this.getCollection(this.tableName)
      .where({
        ...matchCondition,
        dimension: this.fillType,
        start_time: {
    ...
    //活跃用户数
    const statVisitUserRes = await this.getCollection(this.activeUsers.tableName)
      .where({
        ...matchCondition,
        dimension: this.fillType,
      })
    ...
    if (platformInfo.code === 'android') {
      try {
        const statVisitOldDeviceRes = await this.getCollection(this.activeDevices.tableName)
          .where({
            ...data._id,
            create_time: {
              $gte: this.startTime,
  ...
  async fill(data) {
    ...
    // 跳出率
    let bounceTimes = 0;
    const bounceRes = await this.getCollection(this.sessionLog.tableName)
      .where({
        ...matchCondition,
        page_count: 1,
      })
    ...
    //新增用户数
    const uniIDUsers = new UniIDUsers();
    let newUserCount = await uniIDUsers.getUserCount(matchCondition.appid, matchCondition.platform, matchCondition.channel, matchCondition.version, {
      $gte: this.startTime,
      $lte: this.endTime,
    });

    //总用户数
    let totalUserCount = await uniIDUsers.getUserCount(matchCondition.appid, matchCondition.platform, matchCondition.channel, matchCondition.version, {
      $lte: this.endTime,
    });
  ...
  async replenishStat() {
    ...
    //上一次统计时间
    let preStatRes = await this.getCollection(this.tableName)
      .where({
    ...
    // 上一阶段数据
    const preStatData = await this.selectAll(
      this.tableName,
      {
  ...
  async deviceRetentionFillDayly(type, day, date) {
    ...
    const dateTime = new DateTime();
    const { startTime, endTime } = dateTime.getTimeDimensionByType(type, 0 - day, date);
  ...
  async deviceRetentionFillWeekOrMonth(type, day, date) {
    ...
    for (const resultIndex in resultLogRes.data) {
      const resultLog = resultLogRes.data[resultIndex];
      // 获取该批次的活跃设备数
      activeDeviceRes = await this.selectAll(
        activeDevicesObj.tableName,
        {
          appid: resultLog.appid,
      ...
      if (activeDeviceRes && activeDeviceRes.data.length > 0) {
        ...
        // 留存活跃设备数
        const retentionActiveDeviceRes = await this.getCollection(activeDevicesObj.tableName)
          .where({
            appid: resultLog.appid,
            platform_id: resultLog.platform_id,
            channel_id: resultLog.channel_id,
      ...
      // 获取该批次的新增设备数
      const newDeviceRes = await this.selectAll(
        activeDevicesObj.tableName,
        {
          appid: resultLog.appid,
          platform_id: resultLog.platform_id,
      ...
      if (newDeviceRes && newDeviceRes.data.length > 0) {
        ...
        // 新增设备留存数
        const retentionNewDeviceRes = await this.getCollection(activeDevicesObj.tableName)
          .where({
            appid: resultLog.appid,
            platform_id: resultLog.platform_id,
            channel_id: resultLog.channel_id,
  ...
  async userRetentionFillWeekOrMonth(type, day, date) {
    ...
    for (const resultIndex in resultLogRes.data) {
      ...
      // 获取该批次的活跃用户数
      const activeUserRes = await this.selectAll(
        activeUserObj.tableName,
        {
          appid: resultLog.appid,
          platform_id: resultLog.platform_id,
          channel_id: resultLog.channel_id,
      ...
      if (activeUserRes && activeUserRes.data.length > 0) {
        ...
        // 留存活跃用户数
        const retentionactiveUserRes = await this.getCollection(activeUserObj.tableName)
          .where({
            appid: resultLog.appid,
            platform_id: resultLog.platform_id,
            channel_id: resultLog.channel_id,
      ...
      if (thisDayNewUids && thisDayNewUids.length > 0) {
        // 新增用户留存数
        const retentionnewUserRes = await this.getCollection(activeUserObj.tableName)
          .where({
            appid: resultLog.appid,
            platform_id: resultLog.platform_id,
            channel_id: resultLog.channel_id,
