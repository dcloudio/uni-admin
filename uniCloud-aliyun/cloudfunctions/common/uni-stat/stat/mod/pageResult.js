...
module.exports = class PageResult extends BaseMod {
  ...
  async stat(type, date, reset) {
    ...
    const countRes = await this.getCollection(this.pageLog.tableName)
      .where({
        create_time: {
          $gte: this.startTime,
          $lte: this.endTime,
  ...
  async fill(data) {
    ...
    let existTimes = 0;
    const existRes = await this.getCollection(sessionLog.tableName)
      .where({
    ...
    // 跳出率
    let bounceTimes = 0;
    const bounceRes = await this.getCollection(sessionLog.tableName)
      .where({
        appid: data._id.appid,
        version: data._id.version,
        platform: data._id.platform,
