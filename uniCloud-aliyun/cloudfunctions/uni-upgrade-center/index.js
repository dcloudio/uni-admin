...
exports.main = async (event, context) => {
  ...
  switch (event.action) {
    ...
    case 'getAppInfo':
      ...
      const dbAppListRecord = await dbAppList
        .where({
          appid: params.appid,
        })
        .get();
    ...
    case 'getAppVersionInfo':
      ...
      const dbVersionListrecord = await dbVersionList
        .where({
          appid: params.appid,
      ...
      if (dbVersionListrecord && dbVersionListrecord.data && dbVersionListrecord.data.length > 0) return Object.assign({}, dbVersionListrecord.data[0], success);
