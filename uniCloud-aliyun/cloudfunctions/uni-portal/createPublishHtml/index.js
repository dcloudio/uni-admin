...
module.exports = async function (id) {
  ...
  const record = await dbPublishList
    .where({
      _id: id,
    })
    .get({
  ...
  if (record && record.data && record.data.length) {
    ...
    Object.keys(defaultOptions.mpNames)
      .concat('quickapp')
      .forEach((key) => {
        if (!hasValue(appInfo[key])) return;
        imageList.push({
    ...
    if (!(defaultOptions.hasApp || defaultOptions.hasH5 || defaultOptions.hasMP || defaultOptions.hasQuickApp)) {
      return {
        ...fail,
        code: -100,
