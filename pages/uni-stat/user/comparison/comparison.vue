...
<script setup>
  ...
  const getChartDataAction = (query, type = 'day') => {
    ...
    db.collection('uni-stat-result')
      ...
      .groupBy(`platform_id`)
      .groupField(`sum(active_user_count) as ${type}_active_user_count, sum(new_user_count) as ${type}_new_user_count, max(total_users) as ${type}_total_users`)
      .get()
      .then((res) => {
  ...
  const getRangeCountDataAction = (query, type) => {
    ...
    const sub = db
      ...
      .groupField(`sum(active_user_count) as ${type}_active_user_count, sum(new_user_count) as ${type}_new_user_count`)
  ...
  debounceGetState.value = debounce(() => {
    ...
    getRangeCountDataAction(queryState.value, 'month');
  }, 300);
</script>
