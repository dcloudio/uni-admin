...
<script setup>
  ...
  const defaultChartState = ref({
    field: 'new_user_count',
    name: '新增用户',
  });
  const defaultChart = defaultChartState;
  ...
  const getAllDataAction = (query) => {
    ...
    getChartDataAction(query, currentChartTabState.value);
    getTabelDataAction(query);
  };
  const getAllData = getAllDataAction;
  const getChartDataAction = (query, type, name = '日活', field = 'active_user_count') => {
    // this.chartData = {}
    const options = {
      categories: [],
      series: [
  ...
  const getTabelDataAction = (query, field = 'active_user_count') => {
    ...
    db.collection(tableNameState.value)
      ...
      .then((res) => {
        ...
        let daysData = data,
          ...
          weeks = [],
          months = [];
        getRangeCountDataAction(query, 'week').then((res) => {
          const { count, data } = res.result;
          weeks = data;
  ...
  const getRangeCountDataAction = (query, type, field = 'active_user_count') => {
    if (type === 'week') type = 'isoWeek';
    const { pageCurrent } = optionsState.value;
    const db = uniCloud.database();
    return db
  ...
  const mapWithWeekAndMonthAction = (data, weeks, months, field = 'active_user_count') => {
    for (const item of data) {
