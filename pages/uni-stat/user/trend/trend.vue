...
<script setup>
  ...
  const getAllDataAction = (query) => {
    ...
    getPanelDataAction();
    getChartDataAction(query, chartTabState.value, tabNameState.value);
    getTabelDataAction(query);
  };
  const getAllData = getAllDataAction;
  const getChartDataAction = (query, field = chartTabState.value, name = '新增用户') => {
    // this.chartData = {}
