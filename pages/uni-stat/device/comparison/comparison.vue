...
<script setup>
  ...
  const platformsState = ref([]);
  const platforms = platformsState;
  const dayChartsDataState = ref([]);
  const dayChartsData = dayChartsDataState;
  const monChartsDataState = ref([]);
  const monChartsData = monChartsDataState;
  ...
  const dynamicState = {
    ...
    dayChartsData: dayChartsDataState,
    monChartsData: monChartsDataState,
    errorMessage: errorMessageState,
    debounceGet: debounceGetState,
  };
  const chartsDataComputed = computed(() => {
    return [...dayChartsDataState.value, ...monChartsDataState.value];
  });
  const chartsData = chartsDataComputed;
  const versionQueryComputed = computed(() => {
    const { appid } = queryState.value;
  ...
  const getAllDataAction = (query) => {
    ...
    getRangeCountDataAction(query, 'month');
  };
  const getAllData = getAllDataAction;
  const getChartDataAction = (query, type = 'day') => {
    ...
    db.collection('uni-stat-result')
      ...
      .groupBy(`platform_id`)
      .groupField(`sum(active_device_count) as ${type}_active_device_count, sum(new_device_count) as ${type}_new_device_count, max(total_devices) as ${type}_total_devices`)
      .get()
      .then((res) => {
        const data = res.result.data;
        initChartOptionAction(data, 'dayChartsData');
      });
  };
  const getChartData = getChartDataAction;
  const getRangeCountDataAction = (query, type) => {
    query = stringifyQuery(query);
    const db = uniCloud.database();
    const sub = db
      ...
      .groupField(`sum(active_device_count) as ${type}_active_device_count, sum(new_device_count) as ${type}_new_device_count`)
      .orderBy(`year asc, ${type} asc`)
      .get()
      .then((res) => {
        const data = res.result.data;
        initChartOptionAction(data, 'monChartsData', 'month');
      });
  };
  const getRangeCountData = getRangeCountDataAction;
  const initChartOptionAction = (data, goal, type = 'day') => {
    const db = uniCloud.database();
    db.collection('uni-stat-app-platforms')
      .get()
      .then((res) => {
        const options = [
          {
        ...
        dynamicState[goal].value = options;
        ...
        for (const chart of dynamicState[goal].value) {
  ...
  const initChartOption = initChartOptionAction;
  watch(
    () => queryState.value,
    (val) => {
