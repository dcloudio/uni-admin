...
export const sourceMapMethods = {
	...
	async parseError(item) {
		...
		const createSourceMapMatchContext = (context = {}) => Object.assign({
			fileRelative: '',
			cloud_path: '',
			cloud_path_web: '',
			candidateCloudPaths: [],
		...
		const resolveSourceMapUrl = (file, fileName, fileRelative) => {
			const { fileRelative: normalizedFileRelative, cloud_path, cloud_path_web, candidateCloudPaths, fileItem, resolvedUrl } = buildSourceMapMatchContext(file, fileName, fileRelative);
			logSourceMapDebug('parseError:resolve-source-map-url', { base, file, fileName, fileRelative: normalizedFileRelative, cloud_path, cloud_path_web, candidateCloudPaths, matched: !!fileItem, matchedCloudPath: fileItem ? fileItem.cloud_path : '', resolvedUrl });
			return resolvedUrl;
		};
		...
		const requestSourceMapContent = (requestUrl) => new Promise((resolve) => {
			uni.request({
				url: requestUrl,
				dataType: 'string',
		...
		const probeNearbyOriginalPositions = (content, mappingDebugMeta) => {
			...
			offsetCandidates.forEach((offset) => columnCandidates.forEach((column) => {
			...
			attempts.forEach((attempt) => {
				probePromise = probePromise.then((results) => originalPositionFor(content, { line: attempt.line, column: attempt.column }, false).then((mappedPosition) => {
		...
		const fetchSourceMapContent = (sourcemapUrl, mappingDebugMeta = null) => {
			...
			logSourceMapDebug('parseError:get-source-map-content:start', { base, sourcemapUrl, requestUrl, usedMatchedFileUrl: !!(matchedFile && matchedFile.url && requestUrl === matchedFile.url) });
			...
			return requestSourceMapContent(requestUrl).then((result) => {
				...
				logSourceMapDebug('parseError:get-source-map-content:success', { base, requestUrl, statusCode: result.statusCode, contentLength: result.content.length, validSourceMap: isValidSourceMapContent(result.content) });
				...
				return originalPositionFor(result.content, { line: mappingDebugMeta.line + (stacktraceyOptions.lineOffset || 0), column: mappingDebugMeta.column }, false).then((mappedPosition) => {
		...
		if (['harmony', 'harmonyos'].indexOf(String(platformCode).toLocaleLowerCase()) === -1 && ['ios', 'android', 'app'].indexOf(platformCode) > -1) stacktraceyOptions.lineOffset = -1;
		...
		if (rawErrorMessage.includes('UTSError')) {
			...
			preset = {
				...
				parseStacktrace(str) {
					...
					mappingDebugQueue = stack.items.filter((item) => !item.thirdParty).map((item) => ({ file: item.file, fileName: item.fileName, fileRelative: item.fileRelative, line: Number(item.line) || 0, column: Number(item.column) || 0 }));
				...
				getSourceMapContent(file, fileName, fileRelative) {
					...
					return ensureSourceMapFiles(candidateCloudPaths).then(() => {
						...
						const candidateRequestEntries = normalizeCandidatePaths([...matchedCandidateFiles.map((item) => item.url), ...directMapUrls]).map((candidate) => ({
							candidate,
							requestUrl: /^https?:/i.test(candidate) ? candidate : ((findSourceMapFile(candidate) || {}).url || '')
						})).filter((item) => /^https?:/i.test(item.requestUrl));
						if (!candidateRequestEntries.length && mappingDebugMeta) pushUniqueDiagnostic(missingSourceMapFiles, { file: mappingDebugMeta.file, fileRelative: mappingDebugMeta.fileRelative, candidateCloudPaths, directMapUrls });
				...
				asTableStacktrace(payload) {
					logSourceMapDebug('parseError:stack-items-after-map', {
						base,
						items: (payload && payload.stack && Array.isArray(payload.stack.items) ? payload.stack.items : []).slice(0, 10).map((item) => ({ file: item.file, fileRelative: item.fileRelative, fileShort: item.fileShort, line: item.line, column: item.column, parsed: !!item.parsed, beforeParse: item.beforeParse || '' }))
					});
		...
		stacktracey(err, { preset })
			.then((res) => {
				if (typeof res === 'string') {
					...
					if (!rawErrorMessage.includes('UTSError') && stackItemCount && !sourceMapReadFound && (missingSourceMapFiles.length || invalidSourceMapUrls.length || failedSourceMapUrls.length)) this.errMsg = rawErrorMessage;
				}
	...
	async choosefile() {
		...
		inputEl.addEventListener('change', async () => {
			...
			this.logSourceMapDebug('upload:selected-files', { base, count: fileList.length, firstFiles: Array.from(fileList).slice(0, 5).map((file) => file.webkitRelativePath || file.name) });
			...
			const uploadSourceMapTask = (cur) => new Promise((resolve, reject) => {
				// 已上传的文件
	...
	async getSourceMapFileList(obj) {
		...
		this.logSourceMapDebug('parseError:query-source-map-files', { base, candidateCloudPaths, count: result.length, sampleCloudPaths: result.slice(0, 5).map((item) => item.cloud_path) });
		return result;
	}
};
