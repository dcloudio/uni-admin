  ...
  <view>
    ...
    <view class="uni-container">
      ...
      <view class="flex">
        ...
        <uni-datetime-picker
          type="datetimerange"
          v-model="query.pay_date"
          :end="Date.now()"
          return-type="timestamp"
          :clear-icon="true"
          class="uni-stat-datetime-picker"
          @change="dateTabs.index = null"
        />
      </view>
      ...
      >
        ...
        >
          ...
          <uni-tr v-for="(item, index) in data" :key="index">
            ...
            <uni-td align="center">
              <text class="text-btn" @click="pageToOrder(item)"> {{ item.count }} </text>
            </uni-td>
          </uni-tr>
        </uni-table>
...
<script setup>
  ...
  const payDatePickerAction = (val) => {
    queryState.value.pay_date = val;
    searchAction();
  };
  const payDatePicker = payDatePickerAction;
  ...
  const getWhereAction = () => {
    let where = 'status>0';
    let { pay_date, appid, version, uni_platform, channel_code } = queryState.value;
  ...
  const searchAction = () => {
    if (!queryState.value.appid) return;
  ...
  const pageToOrderAction = (item) => {
    let { user_id } = item;
    uni.navigateTo({
      url: `/pages/uni-stat/pay-order/list/list?user_id=${user_id}`,
    });
  };
  const pageToOrder = pageToOrderAction;
  const dateTabsChangeAction = (id, index) => {
    ...
    queryState.value.pay_date = [start, end];
  };
