		...
		<view class="uni-container">
			...
			<uni-table :loading="loading" border stripe :emptyText="$t('common.empty')">
				<uni-tr>
					...
					<uni-th align="center">入口页次数</uni-th>
					<uni-th align="center">跳出率</uni-th>
				...
				<uni-tr v-for="(item ,index) in tableData" :key="index" style="text-align: center; !important">
					...
					<uni-td>{{item.entry_num_visits}}</uni-td>
					<uni-td>{{item.bounce_rate}}</uni-td>
...
<script>
	export default {
		...
		onLoad() {
			this.getData('/pageEnt', 1)
		},
		methods: {
			// 分页触发
			change(e) {
				this.getData('/pageEnt', e.current)
			},
			// 搜索
			search() {
