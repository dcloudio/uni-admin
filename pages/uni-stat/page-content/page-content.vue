...
<script setup>
  ...
  const pageToAction = (url) => {
    uni.redirectTo({
      url,
    });
  };
  const pageTo = pageToAction;
  ...
  const changePageCurrentAction = (e) => {
    ...
    getTableDataAction(queryState.value);
  };
  ...
  const getTableDataAction = (query) => {
    ...
    db.collection(subTableTemp, mainTableTemp)
      .field(`${stringifyField(fieldsMapSource)}, stat_date, page_detail_id`)
  ...
  const getPanelDataAction = (query = stringifyQuery(queryState.value, null, ['uni_platform'])) => {
    let myFieldsMap = JSON.parse(JSON.stringify(fieldsMapSource));
    ...
    const subTable = db
      ...
      .then((res) => {
        ...
        panelDataState.value = mapfields(myFieldsMap, items);
      });
  };
  const getPanelData = getPanelDataAction;
  const inputDialogToggleAction = (url, title) => {
    queryIdState.value = url;
    updateValueState.value = title;
