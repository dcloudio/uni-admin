import { validator, mpPlatform } from '@/js_sdk/validator/opendb-app-list.js';
...
export default {
  ...
  methods: {
    ...
    formatFormData() {
      this.setFormData(
        'screenshot',
        this.screenshotList.map((item) => item.fileID || item.url)
      );
      ...
      this.keepItems = this.platFormKeys
        .filter((key) => this.getPlatformChcekbox(key) && (this.formData[key].url || this.formData[key].abm_url || this.formData[key].qrcode_url))
