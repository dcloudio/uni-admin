	...
	export default {
		...
		methods: {
			...
			ecinit(newVal, oldVal, owner, instance) {
				...
				// }
				const seriesTemplate = cfe.option[cid].seriesTemplate
				for (var i = 0; i < newData.series.length; i++) {
					let temp
					if (Array.isArray(seriesTemplate)) {
						temp = seriesTemplate[i]
					} else {
						temp = seriesTemplate
					}
					temp = temp ? temp : {}
					let Template = rddeepCloneAssign({}, temp, newData.series[i])
					cfe.option[cid].series.push(Template)
				}
