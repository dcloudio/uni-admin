## 0.0.0（2026-09-15）
- 重构 移除 mixin，add/detail 页面改为组合式 API（`<script setup>`），支持 vue3 蒸汽模式
- 新增 删除版本记录时支持同步删除云存储安装包
- 新增 蒸汽模式标记与蒸汽模式上传 wgt 云端生成 SHA256
- 新增 蒸汽模式按平台暂存/恢复资源包：切换平台不丢失已上传包，下载链接与 SHA256 随包恢复
- 新增 废弃资源管理：编辑替换/取消编辑/移除包时标记废弃（obsolete_file_ids），删除记录时同步清理，版本列表支持一键清理废弃资源
## 0.7.0（2024-11-01）
