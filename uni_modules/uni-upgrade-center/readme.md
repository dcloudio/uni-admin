...
- wgt资源包，发布wgt更新，用于App的热更新，可设置是否强制更新，静默更新（uni-app x 的 Vapor 模式应用支持 wgt 热更新，需上传 `.wgt` 格式包并勾选 `Vapor应用`）
- App管理列表及App版本记录列表搜索
...
- #### 发布wgt资源包
	...
	3. `Vapor应用`
		- 发布 uni-app x Vapor 模式应用的资源包时勾选
		- 勾选后上传 `.wgt` 格式的资源包，上传后自动计算 SHA256 供客户端下载后校验完整性
		- Vapor 应用的平台仅支持单选，暂不支持鸿蒙
